...
csvdata2 = pd.read_csv("./result/kal_vehicle_vehicle_command", header=None, skiprows=0,  usecols=[2,3])
...
# '''結果の行列をCSVファイルに書き出し'''
with open("result/kal_vehicle_vehicle_command_result.csv", "w") as f1:
...
px2 = csvdata2[2].values #field.twist.linear.x　テスト対象の出力トピック
py2 = csvdata2[3].values #field.twist.linear.y
...
ax.plot(range(0,len(csvdata2)), px2, linewidth = 2, linestyle = ":", label="Autoware.Auto Node on KALRAY MPPA3-80 Coolidge", color='orange')
ax.grid(True)
...
plt.savefig('result/kal_diff_pure_vehicle_vehicle_command_long_accel_mps2.pdf')
# 表示
plt.show()
...
ax.plot(range(0,len(csvdata2)), py2, linewidth = 2, linestyle = ":", label="Autoware.Auto Node on KALRAY MPPA3-80 Coolidge", color='orange')
ax.grid(True)
ax.legend(bbox_to_anchor=(1, 0), loc='lower right', borderaxespad=0, fontsize=40) #凡例（label='Node'など）を表示
# ax.subplot(1,2,2)
plt.title("vehicle_vehicle_command_diff_front_wheel_angle_rad (pure_pursuit)", fontsize=50, loc='center', y=1.02) #, fontsize=22)
plt.tick_params(labelsize=45)
plt.savefig('result/kal_diff_pure_vehicle_vehicle_command_front_wheel_angle_rad.pdf')
# 表示
plt.show()
